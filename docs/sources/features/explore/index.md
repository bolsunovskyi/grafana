...
For infrastructure monitoring and incident response, you no longer need to switch to other tools to debug what went wrong. Explore allows you to dig deeper into your metrics and logs to find the cause. Grafana's new logging datasource is tightly integrated into Explore and allows you to correlate metrics and logs by viewing them side-by-side. This creates a new debugging workflow where you can:
...
If you just want to explore your data and do not want to create a dashboard then Explore makes this much easier. Explore will if possible show the results as both a graph and a table enabling you to see trends in the data and more detail at the same time.
...
A log query consists of two parts: a log stream selector, and a search expression. For performance reasons you need to start by choosing a log stream by selecting a log label.
...
- `Dedup: exact` Exact matches are done on the whole line, except for date fields.
- `Dedup: numbers` Matches on the line after stripping out numbers (durations, IP addresses etc.).
- `Dedup: signature` The most aggressive deduping - strips all letters and numbers, and matches on the remaining whitespace and punctuation.
